...
abstract class TicketRowBuilder(allTransformations: Map[String, Map[String, TransformationInfo]]) extends Serializable {
  ...
  @transient
  private lazy val Logger: Logger = LoggerFactory.getLogger(classOf[TicketRowBuilder])
  ...
  final def buildRow(ticketInfo: TicketInfo, fields: Seq[String]): Row = {
    ...
    if (rawFields.length < fieldsInfo.size - 3) {
      //FIXME use log (nullpointerexception in distributed mode)
      Logger.warn(s"Expected ${fieldsInfo.size} fields, found ${rawFields.length}: ${rawFields.mkString("||")}. Filling missing fields with nulls.")
      rawFields = rawFields ++ Array.fill(fieldsInfo.size - rawFields.length)(null)
    }

...
abstract class DecimalTransformation extends FieldTransformation {
  ...
  private[this] def getPrecision(args: Seq[String]): Int = args(0).toInt
...
object DecimalFromIntTransformation extends DecimalTransformation {
  ...
  /**
    ...
    */
  override def transform(field: String, args: String*) = if (isNullOrEmpty(field.trim)) null else try BigDecimal(field.trim.toDouble / scala.math.pow(10, args(2).toInt)) catch {
    case e: NumberFormatException => {
      Logger.warn(s"Error parsing decimal $field")
      null
    }
...
object DecimalParseTransformation extends DecimalTransformation {
  override def transform(field: String, args: String*) = if (isNullOrEmpty(field.trim)) null else try BigDecimal(field.trim.replace(args(2)(0), '.')) catch {
    case e: NumberFormatException => {
      Logger.warn(s"Error parsing decimal $field")
      null
    }

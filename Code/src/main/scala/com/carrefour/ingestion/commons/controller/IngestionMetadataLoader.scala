...
object IngestionMetadataLoader {
  ...
  def loadMetadata(settings: IngestionSettings) : Array[IngestionMetadata] = {
    ...
    val metadata = if (settings.entity == "")
      SqlUtils.sql("/Code/src/main/resources/hql/load_BU_Metadata.hql", settings.businessunit) else
      SqlUtils.sql("/Code/src/main/resources/hql/load_Entity_Metadata.hql", settings.businessunit, settings.entity)
    metadata match{
      case Some(metadat) =>

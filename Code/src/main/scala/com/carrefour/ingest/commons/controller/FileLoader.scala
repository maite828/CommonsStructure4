package com.carrefour.ingest.commons.controller

import com.carrefour.ingest.commons.bean.{DelimitedFileType, FileFormats}
import com.carrefour.ingest.commons.core.Context
import com.carrefour.ingest.commons.repository.impl.FileSystemRepositoryImpl
import com.carrefour.ingest.commons.util.SparkJob
import com.carrefour.ingest.commons.util.transform.{FieldInfo, FieldTransformationUtil, TransformationInfo}
...
object FileLoader extends SparkJob[IngestionSettings] {
  ...
  override def run(jobSettings: IngestionSettings): Unit = {
    ...
    val fs = FileSystemRepositoryImpl.dfs
  ...
  def loadDelimitedFile(fileType: DelimitedFileType, settings: IngestionMetadata): Unit = {
    ...
    val input = fileType.fileFormat match {
      case FileFormats.TextFormat => Context.getSparkSession().sparkContext.textFile(settings.inputPath, fileType.numPartitions)
      case FileFormats.GzFormat => Context.getSparkSession().sparkContext.textFile(settings.inputPath)
    ...
    try {
      ...
      val schema = Context.getSparkSession().table(outputTable).schema
      ...
      val df = Context.getSparkSession().createDataFrame(content, schema)
  ...
  def extractFieldsInfo(input: RDD[String], tableName: String, transformations: Map[String, Map[String, TransformationInfo]]): Seq[FieldInfo] = {
    val fieldNames = Context.getSparkSession().table(tableName).schema.fields.map(_.name)

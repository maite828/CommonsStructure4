...
import com.carrefour.ingest.commons.controller.IngestionMetadata
import com.carrefour.ingest.commons.exception.RowFormatException
...
object FieldTransformationUtil {
  ...
  def loadTransformations(transformationsTable: String): Map[String, Map[String, TransformationInfo]] = {
    if (transformationsTable == null || transformationsTable.isEmpty)
      ...
      Context.getSparkSession().table(transformationsTable).rdd.groupBy(_.getAs[String](TableNameField)).map {
